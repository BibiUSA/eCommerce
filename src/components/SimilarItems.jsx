...
export default function(props){
    ...
    while(similarItemsArr.length<5){
        ...
        if(!similarItemsArr.includes(randomSimilar) && thisData[randomSimilar].id != thisItem.id){
            similarItemsArr.push(randomSimilar)
        }
    }

    const similarSuggestion = similarItemsArr.map((num)=>{
        return(<div className="similarItemBox">
                    <Link to ={`/singleitem/${thisData[num].id}`} className="similarItemLink">
                    <img src ={`/${thisData[num].img_src}`} className="similarSuggestionImg"/>
                    <p><strong>{thisData[num].name}</strong></p>
                    <p>{thisData[num].short_description}</p>
                    <p>rating: {thisData[num].ratings}</p>
