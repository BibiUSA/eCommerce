...
export default function(props){
    ...
    const compliment = types.map((type)=>{
        ...
        return <Link to ={`/singleitem/${currentCompliment.id}`} className="complimentLink"><div className="eachCompliment"> 
                <img src={`../${currentCompliment.img_src}`} className="complimentImage"/>
                <p><strong>{currentCompliment.name}</strong></p>
                <p>{currentCompliment.short_description}</p>
                <p>rating: {currentCompliment.ratings}</p>
