...
export default function(props){
    ...
    function adjustQuantity(){
        ...
        let thisObj= cartArray.find((obj)=>obj.size== cartItemObj.size && obj.itemId== cartItemObj.itemId)
        let thisObjIndex= cartArray.findIndex((obj)=>obj.size== cartItemObj.size && obj.itemId== cartItemObj.itemId)
...
return(
    <div className="cartitem">
        <div>
            <Link to={`../singleItem/${fullItemData.id}`} ><img src= {fullItemData.img_src} className="cartItemImage"/></Link>
        </div>
        <div className="cartItemDescription">
            <Link to={`../singleItem/${fullItemData.id}`} className="nameLink"><p className="name">{fullItemData.name}</p></Link>
            <p className="price">{fullItemData.price}</p>
            <p>Size: {cartItemObj.size}</p>
            <p>Quantity: {cartItemObj.quantity}</p>
            <label htmlFor="+"></label><input type="button" value="+" id="+" onClick={adjustQuantity}></input>
            {cartItemObj.quantity>1 && <label htmlFor="-"><input type="button" value="-" id="-" onClick={adjustQuantity}></input></label>}
